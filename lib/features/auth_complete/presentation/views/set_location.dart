...
class _SetLocationState extends State<SetLocation> {
  ...
  void sendLocation() async {
    ...
    // await context.read<ProfileInfoCubit>().submitProfileInfo();
    context.read<ProfileInfoCubit>();

    final state = context.read<ProfileInfoCubit>().state;
    if (state.isSuccess) {
      print('✅ تم إرسال البيانات بنجاح، الانتقال لصفحة رفع الصورة');
      context.pushNamed(RouteNames.upload);
    } else if (state.errorMessage != null) {
      print('❌ خطأ في إرسال البيانات: ${state.errorMessage}');
      ScaffoldMessenger.of(
        context,
